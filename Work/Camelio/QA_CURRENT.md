...
- **테스트 내용은 실제 테스트 수행자가 게임에서 실행하는 구체적인 행동으로 작성한다.**
  - 예: `남은 턴 횟수를 모두 소모`, `[광고 보기]를 선택`, `몬스터를 비치명 피해로 타격`
  - `사망 조건 발생`, `기능 상태 확인`, `정상 동작 확인`처럼 구현 상태나 추상 조건을 행동처럼 쓰지 않는다.
- **기대 결과는 해당 행동 직후 사용자가 실제로 관찰할 수 있는 결과로 작성한다.**
  - 예: `광고 부활 팝업이 표시된다`, `남은 턴 3회가 충전된다`
  - 내부 처리 순서나 구현 개념을 그대로 Expected로 쓰지 않고, 가능하면 화면·수치·상태 변화로 표현한다.
- **사전 조건에는 실제 분기를 만들기 위해 필요한 상태만 둔다.**
  - 테스트 결과에 영향을 주지 않는 조건은 넣지 않는다.
- **우선순위·조건 판정·상태 전이도 추상적인 규칙 확인 TC로 만들기보다, 실제 행동과 그 결과로 검증한다.**
  - 예: `무료 부활이 광고 부활보다 우선한다`는 규칙은 `남은 턴 횟수를 모두 소모 → 광고 팝업 없이 무료 부활`처럼 검증한다.
- 동일 원칙을 `TC_[기능]`, `CL_[기능]` 등 향후 모든 QA 실행 자산에 일관되게 적용한다.
